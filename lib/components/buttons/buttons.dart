...
enum _ButtonLayoutMode { inherited, adaptive, custom }

class ButtonLayout extends StatelessWidget {
  const ButtonLayout({super.key, this.spacing, this.icon, this.label})
    : assert(icon != null || label != null);

  const ButtonLayout.adaptive({super.key, this.spacing, this.icon, this.label})
    : assert(icon != null || label != null);

  const ButtonLayout.custom({super.key, this.spacing, this.icon, this.label})
    : assert(icon != null || label != null);
  ...
  Widget build(BuildContext context) {
    final scope = _ButtonScope.maybeOf(context);
...
class Button<S extends Object?> extends StatefulWidget {
  const Button({
    super.key,
    required this.style,
  ...
  final ButtonStyleConcrete<S> style;
  final S settings;
  ...
  @override
  State<Button<S>> createState() => _ButtonState<S>();
}

class _ButtonState<S extends Object?> extends State<Button<S>>
  ...
  @override
  void didUpdateWidget(covariant Button<S> oldWidget) {
...
class _ButtonScope extends InheritedWidget {
  const _ButtonScope({
    super.key,
  ...
  @override
  bool updateShouldNotify(_ButtonScope oldWidget) =>
      iconLabelSpace != oldWidget.iconLabelSpace;

  @override
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    super.debugFillProperties(properties);
    properties.add(DoubleProperty("iconLabelSpace", iconLabelSpace));
  }

  static _ButtonScope? maybeOf(BuildContext context) =>
      context.dependOnInheritedWidgetOfExactType<_ButtonScope>();

  static _ButtonScope of(BuildContext context) {
...
/// A widget to pad the area around a [Button]'s inner [Material].
///
